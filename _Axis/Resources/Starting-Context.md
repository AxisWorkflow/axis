...
<!-- compiled: 2026.09.23.17.24.17.518Z -->
...
- Before any checkpoint or Step 1 of a valid production Command, including `^^`, apply its role gate. Main alone follows [Practices > Reminders > Surfacing and Session Checkpoint]: inspect open Reminder metadata against `reminder-check`, surface only newly due or newly created/rescheduled already-due Subjects, and advance the Flag only after trustworthy current UTC is established. The Command then continues normally. `^reminders` may fold this checkpoint into its own queue read. An empty queue is silent. External Agents may inspect due metadata read-only and deduplicate in conversation; they never advance the shared `reminder-check` Flag. Subagents do not run the global checkpoint. An allowed additive Command never authorizes a later maintenance phase that the role forbids; route that phase to Main by Request.
...
- A validated project overlay may declare additional command-to-file mappings after normal Main Session Start. Resolve only the exact mappings in the loaded overlay; never discover Commands by scanning its directory. Before dispatch, require `session-id` and `project-overlay` to name the same visible normal Session ID and require the overlay ID and path to match the revalidated declaration. A missing, malformed, foreign-session, or stale cache makes every overlay Command unavailable without affecting production Commands. External Agents and Subagents never dispatch overlay Commands.
