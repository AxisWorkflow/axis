...
External Agents may read due metadata and deduplicate in their conversation, but never create, clear, rewrite, or advance `reminder-check`, even when it names another session. Main Agent alone mutates Reminders. Subagents return candidates to Main; External Agents use Requests.
