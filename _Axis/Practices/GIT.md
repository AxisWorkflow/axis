...
- `received` means incoming ciphertext was verified and installed through a verified pre-change copy plus checked exit rollback.
...
The gitignored `.binding` stores only content digests needed to recognize a safe round trip. A full-folder copy carries it; a clone reconstructs it after the first verified receive. The helper decrypts in permission-restricted operating-system temporary storage and makes a verified pre-change plaintext/binding copy under `_Axis/Secrets/.recovery.*/` (mode 700) before mutation. An ordinary error or trapped exit attempts rollback, checks every operation, and verifies the restored plaintext digest and binding bytes. Only verified rollback or successful receive permits recovery-copy deletion. A failed rollback returns exit 4 and `error:recovery-required`, preserves the last complete originals and binding in that protected ignored directory, and blocks subsequent initialization, receive, or seal. `status` reports `recovery-required`; any such token overrides an earlier ordinary error. Do not retry a transfer, claim restoration, or clean the recovery directory. Stop Secrets synchronization, report only the recovery state (no names, values, or paths), and have the User restore from the retained original copy through a trusted local surface. Verify the saved `original-digest` and binding before explicitly removing recovery residue. Untrappable process or machine failure takes the same recovery-required path on the next invocation; unrelated project work may continue.
