...
- any project-overlay declaration and its declared project file still match their pre-update hashes unless the preview named and User approved an explicit semantic migration; the current session never loads them, and the required fresh session revalidates them;
