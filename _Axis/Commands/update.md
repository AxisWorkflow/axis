...
The live project is `local`. If the base tag cannot be obtained, or the target does not contain the installed or provisional origin version in its changelog history, stop for a manual migration. A same-version target is a clean no-op: report that Axis is current and STOP. Refuse a downgrade. Every date-style version is older than every `MAJOR.MINOR` version. Compare `MAJOR.MINOR` versions by integer major and then integer minor; compare date-style versions by their numeric date components and then the numeric same-day sequence, treating no suffix as sequence 1; never compare version strings lexically (`1.10` is newer than `1.09`, and `-10` is newer than `-2`).
