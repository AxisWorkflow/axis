...
2. If the command has no additional text: present the Tasks grouped by Status (**Active** and **Blocked** first, then **Completed** and **Cancelled**), each with its name, label, age, and Initiative key when present. Do not count an Initiative as another Task. Then STOP.
...
3. If the command has additional text: interpret it as an update instruction (e.g., "complete Draft-Report", "block Survey on vendor reply", "add a task to review pricing").
	...
	- b. Preserve or update optional Initiative membership under [Practices > Initiatives] when the requested change affects it, including its reciprocal Task link. Apply it per [Practices > Tasks] - index entry and detail file together; write or advance `updated:` for every material change; respect the WORM rules for terminal Tasks.
